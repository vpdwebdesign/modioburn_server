...
bool User::phoneTaken(const QString &phone)
{
    QSqlQuery q;
    q.prepare(QLatin1String("SELECT phone FROM users "
                            "WHERE phone = :phone"));
    ...
    if (q.exec())
    {
        if (q.isActive() && q.isSelect()) {
            ...
            if (q.isValid())
            {
                if (q.value(0).toString() == phone)
                {

                    qDebug() << "Phone number:" << phone << "already registered.";
                    return true;
                }
                else {
                    qDebug() << "Phone:" << phone << ". No such phone number.";
                    return false;
                }
            }
            else
...
bool User::emailTaken(const QString &email)
{
    QSqlQuery q;
    q.prepare(QLatin1String("SELECT email FROM users "
                            "WHERE email = :email"));
    ...
    if (q.exec())
    {
        if (q.isActive() && q.isSelect()) {
            ...
            if (q.isValid())
            {
                if (q.value(0).toString() == email)
                {

                    qDebug() << "Email:" << email << "already registered.";
                    return true;
                }
                else {
                    qDebug() << "Email:" << email << ". No such email.";
                    return false;
                }
            }
            else
...
bool User::getUser(const QString &username)
{
    ...
    if (firstQ.exec())
    {
        if (firstQ.next())
        {
            m_userId = firstQ.value(0).toInt();
            m_username = firstQ.value(1).toString();
        }
        else
        {
            qDebug("getUser() Error: cannot get next value");
        }
    }
    else
    {
...
bool User::verifyPhone(const QString &username, const QString &phone)
{
    QSqlQuery q;
    q.prepare(QLatin1String("SELECT phone FROM users "
                            "WHERE id IN "
        ...
        if (q.isActive() && q.isSelect()) {
            ...
            if (q.isValid())
            {
                if (q.value(0).toString() == phone)
                {
                    ...
                    qDebug() << "Phone number matches";
                    return true;
                }
                else {
                    qDebug() << "Phone number does not match";
                    return false;
                }
            }
            else
